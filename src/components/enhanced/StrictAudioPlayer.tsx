...
interface StrictAudioPlayerProps {
  audioUrl: string;
  onComplete?: () => void;
...
const StrictAudioPlayer = ({ 
  audioUrl, 
  onComplete, 
  defaultRepeatCount = 5,
  autoPlay = false,
  debugMode = false
}: StrictAudioPlayerProps) => {
  ...
  const debugLog = (message: string, data?: any) => {
    if (debugMode) {
      console.debug(`[StrictAudioPlayer] ${message}`, data || '');
    }
  };
  ...
  useEffect(() => {
    // Update playback state when repeat count changes
  ...
  const initializeAudio = () => {
    ...
    // Auto-play if enabled and this is the first load
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current && audioRef.current.duration) {
      ...
      // Calculate overall progress across all repeats
      const targetRepeats = playbackStateRef.current.targetRepeats;
      const completedRepeats = playbackStateRef.current.currentRepeat - 1;
  ...
  const handleAudioEnded = () => {
    ...
    // STRICT VERIFICATION: Only proceed if we haven't completed and haven't exceeded target
    if (state.hasCompleted || state.currentRepeat >= state.targetRepeats) {
      debugLog('Playback complete - stopping', {
        reason: state.hasCompleted ? 'already completed' : 'reached target repeats'
      });
      setIsPlaying(false);
    ...
    // Increment repeat count and continue
    state.currentRepeat++;
    setCurrentRepeat(state.currentRepeat);
    
    debugLog('Starting next repeat', {
      newRepeat: state.currentRepeat,
      remaining: state.targetRepeats - state.currentRepeat + 1
    });

    // Brief pause between repetitions for better UX
    setTimeout(() => {
  ...
  const handleRepeatCountChange = (newCount: string) => {
    ...
    // Reset if currently playing to apply new count
    if (isPlaying) {
      handleReset();
    }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400">
          <span>
            Repeat {currentRepeat} of {targetRepeats}
            {isComplete && ' - Complete!'}
          </span>
...
export default StrictAudioPlayer;