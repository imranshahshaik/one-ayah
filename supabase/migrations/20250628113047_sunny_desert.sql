/*
  # Enhanced Quran Database Schema

  1. New Tables
    - `surahs` - Complete list of all 114 Quran chapters
    - `ayahs` - Individual verses with Arabic, English, transliteration
    - `memorized_ayahs` - User memorization tracking with enhanced fields
    - `user_progress` - Enhanced progress tracking with streak counts

  2. Security
    - Enable RLS on all tables
    - Public read access to surahs and ayahs
    - User-specific access to personal memorization data
  ...
  4. Sample Data
    - All 114 surahs with proper names and metadata
    - Sample ayahs from Al-Fatihah for testing
*/
...
-- Create enhanced memorized_ayahs table (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'memorized_ayahs') THEN
...
-- Create enhanced user_settings table (only if it doesn't exist)
DO $$
BEGIN
...
-- Create page_completions table (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'page_completions') THEN
...
-- Create daily_sessions table (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'daily_sessions') THEN
...
-- Add new columns to existing user_progress table if they don't exist
DO $$
BEGIN
  ...
  -- Add ayahs_due_review column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Add pages_completed column if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'memorized_ayahs') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'page_completions') THEN
    ALTER TABLE public.page_completions ENABLE ROW LEVEL SECURITY;
  END IF;
  
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'daily_sessions') THEN
    ALTER TABLE public.daily_sessions ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

-- Create policies for public read access to surahs and ayahs (with conflict handling)
DO $$
BEGIN
  -- Surahs policies
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'surahs' AND policyname = 'Allow public read on surahs'
  ) THEN
  ...
  -- Ayahs policies
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'ayahs' AND policyname = 'Allow public read on ayahs'
  ) THEN
...
-- Create policies for memorized_ayahs (with conflict handling)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'memorized_ayahs') THEN
    -- SELECT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'memorized_ayahs' AND policyname = 'Users can view own memorized ayahs'
    ) THEN
    ...
    -- INSERT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'memorized_ayahs' AND policyname = 'Users can insert own memorized ayahs'
    ) THEN
    ...
    -- UPDATE policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'memorized_ayahs' AND policyname = 'Users can update own memorized ayahs'
    ) THEN
...
-- Create policies for user_settings (with conflict handling)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_settings') THEN
    -- SELECT policy
...
-- Create policies for page_completions (with conflict handling)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'page_completions') THEN
    -- SELECT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'page_completions' AND policyname = 'Users can view own page completions'
    ) THEN
    ...
    -- INSERT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'page_completions' AND policyname = 'Users can insert own page completions'
    ) THEN
...
-- Create policies for daily_sessions (with conflict handling)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'daily_sessions') THEN
    -- SELECT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'daily_sessions' AND policyname = 'Users can view own daily sessions'
    ) THEN
    ...
    -- INSERT policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'daily_sessions' AND policyname = 'Users can insert own daily sessions'
    ) THEN
    ...
    -- UPDATE policy
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE tablename = 'daily_sessions' AND policyname = 'Users can update own daily sessions'
    ) THEN
...
-- Insert complete surahs data with proper escaping
